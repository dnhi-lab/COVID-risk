...
# Rainclout plot, y = Absolute risk perception, x = time horizon (2 weeks, 2 months, next year, lifetime)
...
# load data
data_get_covid_abs <- read_table2("data/get_COVID_self_time_horizon.txt")
head(data_get_covid_abs)


# PREPROCESS DATA --------------------------------------------------------------------------------------

# make data frame 
data_gca <- data.frame(data_get_covid_abs)

# add subject number
sub_n <- length(data_gca[,1])
subj <- 1:sub_n
data_gca <- data.frame(subj, data_gca)
...
data_gca$time <- factor(data_gca$time) 

# change order of factor levels 
data_gca$time <- factor(data_gca$time, levels = c("X2_weeks","X2_months","Next_year","Lifetime"))
...
fig_gca <- ggplot(data = data_gca, aes(x = time, y = response)) + 
  geom_point(size = 1.5,
             color = color1,
             alpha = 0.6,
